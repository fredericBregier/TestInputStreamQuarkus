 ...
 *
 * @author guss77
 *     Updated and adapted by F.Bregier but not as efficient as Netty version
 */
public class VertxToInputStream extends InputStream implements WriteStream<Buffer> {
  ...
  public VertxToInputStream(Vertx vertx) {
    context = vertx.getOrCreateContext();
  }

  public VertxToInputStream(Vertx vertx, int writeQueueMaxSize) {
    context = vertx.getOrCreateContext();
  ...
  public VertxToInputStream wrap(OutputStream os) {
  ...
  private class PendingWrite {
    ...
    public int readNext() {
      ...
      int val = 0xFF & data.getByte(
          position++); // get byte's bitwise value, which is what InputStream#read() is supposed to return
